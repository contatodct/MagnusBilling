...
class Report2 extends FPDF
{
    ...
    public function Header()
    {
        if ($this->PageNo() === 1) {
            ...
            if (strlen($this->logo) > 10) {
                $this->Image($this->logo, 10, 14, 40);
            }

            if (strlen($this->logo2) > 10) {
                $this->Image($this->logo2, 160, 8, 40, 0, 'png');
            }
            $this->Ln(40);
            $this->SetFont($this->fontFamily, 'B', $this->fontSize + 1);
            $this->Cell(0, 6, utf8_decode(' Realize o pagamento exclivamente via PIX, basta scanear o QRCode.'), 1, 0, 'C');
            $this->Ln(7);
            ...
            $userDetails = "DADOS DO CLIENTE \n";
            //$userDetails = $this->SetFont($this->fontFamily, '', $this->fontSize);
            ...
            if (isset($this->userDoc)) {
                $userDetails .= $this->userDoc . "\n ";
            } else {
                $userDetails .= "CPF/CNPJ:\n";
            }
            if (isset($this->address)) {
                $userDetails .= $this->address . "\n";
            } else {
                $userDetails .= "Endereço:\n";
            }
            if (isset($this->email)) {
            ...
            $this->MultiCell(100, 4, $userDetails, 1, 'L');
            $this->Ln(6);
            ...
            $this->SetFont($this->fontFamily, 'B', $this->fontSize);
            $invoiceDetails = "RESUMO GERAL DA FATURA\n";

            $invoiceDetails .= $this->title . "\n";
            $invoiceDetails .= $this->subTitle . "\n";
            $invoiceDetails .= $this->subTitle2 . "\n";
            $invoiceDetails .= $this->subTitle3 . "\n";
            $invoiceDetails .= "\n";

            $this->MultiCell(190, 4, $invoiceDetails, 1, 'R');
            $this->Ln(6);
    ...
    public function bodyExtra()
    {
        ...
        $widthContent = $this->orientation === 'P' ? 190 : 276;
