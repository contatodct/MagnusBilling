...
if (!isset($modelUser->email) || strlen($modelUser->email) < 10 || !preg_match("/@/", $modelUser->email)) {
    echo "<div id='load' > " . Yii::t('zii', 'Você precisa dastrar um email') . "</div> ";
    return;
}

if (!isset($modelUser->doc) || strlen($modelUser->doc) < 10) {
    $modelUser->doc = $modelUser->username;
}
...
if (!isset($_GET['id'])) {
    ...
    $items = [];
    $i     = 1;
    foreach ($services as $iten_name => $price) {

        $price = preg_replace("/\.|\,/", '', $price);

        $items[] = [
            'name'   => urldecode($iten_name), // nome do item, produto ou serviço
            'amount' => 1, // quantidade
            'value'  => intval($price), // valor (1000 = R$ 10,00)
        ];
    ...
    $metadata = array('notification_url' => $protocol . $_SERVER['HTTP_HOST'] . '/mbilling/index.php/gerencianet?id_user=' . $modelUser->id . '&id=' . time() . '&amount=' . $amount);
